...
We want to execute our dev job from the CLI. Because our Python file has two jobs, we need to wrap it in a repository to determine which one to load when pointing to it from out of process:

```python file=/intro_tutorial/advanced/pipelines/modes.py startafter=start_repo_marker endbefore=end_repo_marker
@repository
def dev_repo():
    return [calories_dev_job]
```

Then we can execute it from the command line:

```bash
dagster job execute -f modes.py -c resources.yaml -r dev_repo
```
...
```python file=/intro_tutorial/advanced/pipelines/presets.py startafter=start_repo_marker endbefore=end_repo_marker
@repository
def dev_repo():
    return [calories_dev_job]
```
...
```bash
dagster job execute -f modes.py -r dev_repo
```
